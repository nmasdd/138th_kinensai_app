...
- デプロイ手順 (`kinensai_app/` で、要 `npx wrangler login`):
  ...
  npx wrangler deploy
  ```
  `dist/` は gitignore だがデプロイ入力のため export で再生成すること。`--dry-run` で事前検証可。`wrangler` は devDependencies に入れていないため、初回は `npx --yes wrangler@4 deploy` のように実行する (ログインは `npx wrangler login`)。
- Workers Builds (push で自動デプロイ) — **設定済み (2026-09-16)**:
  - Worker `138-kinensai-app` (script tag `195ac5aeef584a4f8e3d93dafa17b1bd`) ← GitHub `nmasdd/138th_kinensai_app` (branch `main`、provider_account_id `114152773` / repo_id `1273903080`、repo_connection `4395c41d-d499-4513-9dfd-89e36595d563`)
  - Build 設定: root directory `kinensai_app` / build command `npx expo export --platform web` / deploy command `npx --yes wrangler@4 deploy` / build caching 有効 (依存は Builds が `npm ci` で自動インストール。所要 約2〜3分)
  - トリガー: main = 「本番 (main) へプッシュでデプロイ」`61de875b-0db7-4e70-ab00-87acc5969a71`、非本番 = 「非本番ブランチ (プレビュー)」`29a4922b-4751-4f65-9248-a68ad539b4ad` (`npx --yes wrangler@4 versions upload`)
  - 変更・実行は API から: `PATCH /accounts/{id}/builds/workers/{script_tag}`、`PATCH /accounts/{id}/builds/triggers/{uuid}`、手動ビルド `POST /accounts/{id}/builds/triggers/{uuid}/builds` (`{ "branch": "main" }`)
  - GitHub 連携は初回のみダッシュボードでの認可が必要 (未認可で `PUT /accounts/{id}/builds/repos/connections` すると `8000008: This project is disconnected from your Git account`)。参考: 既存 `137kinensai-app` は `chitarom` (account id 173138402) / repo id 1042723614、`138th-kinensai` は `ypaakamikaze-art` / `HP` に接続済み。
