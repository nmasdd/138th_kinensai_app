...
- Workers Builds (push で自動デプロイ): Worker `138-kinensai-app` (script tag `195ac5aeef584a4f8e3d93dafa17b1bd`) を GitHub `nmasdd/138th_kinensai_app` (main) に接続する。Builds 設定は root directory `kinensai_app`、build command `npx expo export --platform web`、deploy command `npx --yes wrangler@4 deploy` (依存は Builds が自動インストール)。**GitHub の認可は初回のみダッシュボードで必要** (Workers & Pages → `138-kinensai-app` → Settings → Builds → Git に接続)。未認可の状態で API `PUT /accounts/{id}/builds/repos/connections` を呼ぶと `8000008: This project is disconnected from your Git account` で失敗する。認可後は同 API (provider_type `github`・provider_account_id は GitHub の数値ID `114152773`・repo_id はリポジトリの数値ID `1273903080`) で接続を作成し、`POST /accounts/{id}/builds/triggers` で main ブランチのトリガーを登録できる。参考: 既存 `137kinensai-app` は `chitarom` (account id 173138402) / repo id 1042723614 に接続済み。
