...
- `src/data/timetable.ts` parses `time/Auditorium.csv` / `time/Stage.csv` (4-col `team,day,start,end`, `day: 0=土,1=日`; 5-col also accepted). Stage performances repeat per day, so the stage CSV has one row per appearance and its IDs are prefixed `stage-` (shared `timetable-overrides.json` / `delays.json` are split by that prefix). `delayMinutes` defaults to 0 — realtime delay feed goes here. Stage/auditorium group JSONs (`stage-groups.json`, `auditorium-groups.json`) hold meta only (intro/genre/image); times come from the CSVs.
