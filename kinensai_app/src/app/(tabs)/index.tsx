    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        ...
        <Rise delay={80}>
          ...
          <Text style={[type.titleMedium, { color: m3.onSurface }]} accessibilityRole="header">いま開催中</Text>
          {isLoading ? (
            <M3LoadingView />
          ) : nowItem ? (
            <Text style={[type.bodyMedium, { color: m3.onSurface, marginTop: 8 }]} accessibilityLiveRegion="polite">
              講堂: {nowItem.team} ({nowItem.start}–{nowItem.end})
            </Text>
          ) : (
            <Text style={[type.bodyMedium, { color: m3.onSurfaceVariant, marginTop: 8 }]} accessibilityLiveRegion="polite">
              講堂・ステージの現在演目は確認中です
            </Text>
          )}
        </M3Card>
        ...
        <Rise delay={200}>
          ...
          {festival.notes.map((n) => (
            <View key={n.title} style={styles.note}>
              <Text style={[type.titleSmall, { color: m3.onSurface }]}>{n.title}</Text>
              <Text style={[type.bodyMedium, { color: m3.onSurfaceVariant }]}>{n.body}</Text>
            </View>
          ))}
        </M3Card>
        </Rise>
